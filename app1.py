...
# Function to train ARIMA or SARIMA model
# def train_model(stock_name, model_type):
#     stock_data = df[df["Stock"] == stock_name].sort_values("Date")
#     close_prices = stock_data["Close"]

#     exog_features = stock_data["Sentiment_Score"] if "Sentiment_Score" in stock_data.columns else None
...
# Function to forecast stock price
...
#         st.warning(f"⚠️ No {model_type} model found for {stock_name}. Training a new one...")
    ...
    # # Generate future exogenous values (repeat last available value)
    # if "Sentiment_Score" in stock_data.columns:
    #     last_exog_value = stock_data["Sentiment_Score"].iloc[-1]  # Get last known value
    #     future_exog_values = [last_exog_value] * periods  # Repeat for forecast steps
    # else:
...
#  st.header("🔹 Select Stock & Model")
#  stock_name = st.selectbox("Select Stock", stocks)
#  model_choice = st.radio("Choose Model", models)
#  if st.button("Predict Next 15 Days"):
#     if st.button("Predict Next 15 Days"):
#      plot_forecast(stock_name, model_choice)
    # st.write(f"📊 Forecasting {stock_name} using {model_choice}...")

    # # Generate Predictions
    # forecast_df = forecast_stock_price(stock_name, model_choice)

    # # Merge with Original Data
    # full_data = df[df["Stock"] == stock_name][["Date", "Close"]].rename(columns={"Close": "Actual Price"})
    # full_data = pd.concat([full_data, forecast_df], ignore_index=True)

    # # Plotly Graph
    # fig = go.Figure()
    # fig.add_trace(go.Scatter(x=full_data["Date"], y=full_data["Actual Price"], mode="lines", name="Actual Price", line=dict(color="blue")))
    # fig.add_trace(go.Scatter(x=forecast_df["Date"], y=forecast_df["Predicted_Price"], mode="lines+markers", name="Predicted Price", line=dict(color="red", dash="dash")))

    # fig.update_layout(title=f"{stock_name} Price Prediction ({model_choice})", xaxis_title="Date", yaxis_title="Stock Price", template="plotly_dark")
...
#  st.subheader(f"📊 Latest Stock Data for {stock_name}")
#  st.dataframe(df[df["Stock"] == stock_name].tail(10))

# Additional Features (Placeholder for Future Enhancements)
#  st.sidebar.header("🔹 Additional Insights")
#  if st.sidebar.checkbox("Show Moving Averages"):
#     st.write("📌 Feature Coming Soon: Moving Averages Analysis")

#  if st.sidebar.checkbox("Show Sentiment Trends"):
#     st.write("📌 Feature Coming Soon: News Sentiment Analysis")
