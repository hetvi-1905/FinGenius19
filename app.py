...
if page == "Home":
    ...
    if ticker:
    # Fetch historical stock data for the last month
        stock = yf.Ticker(ticker)
        hist = stock.history(period="1mo")

    # Plot stock prices with markers for each day's closing price
        fig = px.line(hist, x=hist.index, y="Close", 
                  title=f"{ticker} Stock Prices (Last Month)", 
                  markers=True)  # Enables markers on data points
    ...
    st.write(f"**Risk Score:** The risk to invest in this stock is {risk_score}")
